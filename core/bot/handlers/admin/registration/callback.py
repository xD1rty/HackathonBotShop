...
async def ban_user(call: CallbackQuery, bot: Bot, session: AsyncSession):
...
async def accept_user(call: CallbackQuery, bot: Bot, session: AsyncSession):
    ...
    await call.message.answer(
        admin_user_request_accept.format(name=user.name, position=user.position, id=telegram_user_id,
